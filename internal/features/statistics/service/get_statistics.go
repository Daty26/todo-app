...
func (s *StatisticsService) GetStatistics(
	...
	if from != nil && to != nil {
		if to.Before(*from) || to.Equal(*from) {
			return domain.Statistics{}, fmt.Errorf("`to` must be after `after`: %w", core_errors.ErrInvalidArgument)
		}
	}
...
func calcStatistics(tasks []domain.Task) domain.Statistics {
	...
	return domain.NewStatistics(tasksCreated, tasksCompleted, &tasksCompletionRate, tasksAverageCompletionTime)
}
