...
func NewTask(
	...
	return Task{
		...
		Completed:   completed,
		CreatedAt:   createdAt,
		CompletedAt: completedAt, AuthorUserID: authorUserID,
	}
}
...
func (p *TaskPatch) Validate() error {
	if p.Title.Set && p.Title.Value == nil {
		return fmt.Errorf("`Title` can't be patche to NULL: %w", core_errors.ErrInvalidArgument)
	}
	if p.Completed.Set && p.Completed.Value == nil {
		return fmt.Errorf("`Completed` can't be patche to NULL: %w", core_errors.ErrInvalidArgument)
	}
...
func (t *Task) ApplyPatch(patch TaskPatch) error {
	...
	if err := tmp.Validate(); err != nil {
		return fmt.Errorf("valiade patch task: %w", err)
	}
	*t = tmp
	return nil
}
